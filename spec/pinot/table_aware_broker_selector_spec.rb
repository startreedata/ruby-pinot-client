RSpec.describe Pinot::TableAwareBrokerSelector do
  ...
  describe "#init" do
    it "raises NotImplementedError because subclasses must implement it" do
      expect { described_class.new.init }
  ...
  describe "#select_broker" do
    ...
    it "selects from all brokers for empty table name" do
      ...
      expect(broker).to(satisfy { |b| ["host1:8000", "host2:8000"].include?(b) })
    end
    ...
    it "raises for unknown table" do
      expect do
        selector.select_broker("unknownTable")
      end.to raise_error(Pinot::TableNotFoundError,
                         /unable to find table: unknownTable/)
    end

    it "raises for table with empty broker list" do
      expect do
        selector.select_broker("emptyTable")
      end.to raise_error(Pinot::BrokerNotFoundError,
                         /no available broker for table: emptyTable/)
    end
